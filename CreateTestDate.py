...
class MyTestCase():
    ...
    def test_4_CrecruitAdd(self):
        ...
        self.log.info('学员报读院校id：' + unvsid)
        pfsnId = extFile.getYamlParms(('CStudent', 'pfsnId'))
        self.log.info('学员报读专业pfsnId：' + pfsnId)
        taId = extFile.getYamlParms(('CStudent', 'taId'))
        self.log.info('学员报读地区：' + taId)
    ...
    def test_7_cj_fd_pay(self):
        ...
        keys = (('data', 'learnId'), ('data', 'payData', 'learnId'), ('data', '_web_token'), ('data', 'grade'),
                ('data', 'payData', 'grade'), ('data', 'payableCount'), ('data', 'payData', 'payAmount'), ('data', 'payData', 'items', 0, 'payAmount'),
                ('data', 'payData', 'items', 0, 'amount'),
    ...
    def test_I_updateStudentScore(self):
        ...
        case = TestCaseAssembly().setAipParam('updateStudentScore',
                                              (learnId, web_token, courseId0, courseName0, courseId1, courseName1, courseId2, courseName2),
                                              (('data', 'learnId'), ('data', '_web_token'), ('data', 'scores[0].courseId'), ('data', 'scores[0].courseName'),
                                               ('data', 'scores[1].courseId'), ('data', 'scores[1].courseName'), ('data', 'scores[2].courseId'),
                                               ('data', 'scores[2].courseName')))
        response = YzApi().lapi(method=case[0], headers=case[1], urls=case[2], data=case[3])
    ...
    def test_J_stdEnroll_enroll(self):
        ...
        grade = intFile.getYamlParms(('CJ', 'grade'))
        pfsnId = extFile.getYamlParms(('CStudent', 'pfsnId'))
        unvsId = extFile.getYamlParms(('CStudent', 'unv sId'))
    ...
    def testing(self,unvsid=None):
        if unvsid is None:
            DataSource().setLearnInfo('CStudent','unvsId',unvsid)
        else:
            self.test_0_sUnvs()
        self.test_1_sPfsnByOnScholarship()
        self.test_2_getOpenTestAreaByCity()
        self.test_3_showFeeList()
        self.test_4_CrecruitAdd()


if __name__ == '__main__':
    num = sys.argv[1]
    MyTestCase().test_00_login()
    i = int(num)
    while i > 0:
        MyTestCase().testing()
        i -= 1
