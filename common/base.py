...
class YzApi():
    ...
    def login(self):
        try:
            if self.Yaml.get('urls') != None:
                r=requests.session().post(url=self.DataSource.getBaseUrl()+self.Yaml.get('urls'), data=self.Yaml.get('data'), headers=self.DataSource.getHearder('login'))
    ...
    def lapi(self, method, urls, data=None, headers=None, json=None):
        if method == "post" or method == "POST":
            try:
                r = requests.post(url=self.DataSource.getBaseUrl()+urls, json=json,data=data, headers=headers,cookies = self.DataSource.getCookie())
                r.encoding = 'utf-8'
        ...
        if method == "get" or method == "GET":
            try:
                r = requests.get(url=self.DataSource.getBaseUrl()+urls, params=data, headers=headers,cookies = self.DataSource.getCookie())
                r.encoding = 'utf-8'
                self.log.info(method+' '+self.DataSource.getBaseUrl()+urls + ":请求发送成功！")
                return r
            except Exception as e:
    ...
    def getBaseinfo(self, type):
        try:
            ...
            while (i):
                ...
                r = requests.post(url=self.DataSource.getBaseUrl()+url, data=data, headers=self.Yaml.get('hearders'),cookies = self.DataSource.getCookie())
                r.encoding = 'utf-8'
